...
exports.getOnlineProfile = async (req, res) => {
  try {
    const { checkOnline } = req.params;

    if (checkOnline === "online") {
      const drivers = await Driver.find({ online_status: true }).select(
        "-password_hash"
      );

      if (!drivers || drivers.length === 0) {
        return res.status(404).json({ error: "No online drivers found" });
      }
      ...
      return res
        ...
        .json({
          error:
            'Invalid parameter. Please pass "online" to fetch online drivers.',
        });
    }
  } catch (error) {
...
exports.updateProfile = async (req, res) => {
  ...
  try {
    ...
    if (req.body.phone_number) {
      const existingDriverWithPhoneNumber = await Drive.findOne({
        phone_number: req.body.phone_number,
      });
      if (
        existingDriverWithPhoneNumber &&
        existingDriverWithPhoneNumber._id.toString() !== req.params.id
      ) {
        return res
          .status(400)
          .json({
            error: "Phone number is already registered with another driver",
          });
      }
    ...
    if (req.body.license_number)
      updates.license_number = req.body.license_number;
    ...
    if (req.body.vehicle_plate_number)
      updates.vehicle_plate_number = req.body.vehicle_plate_number;
    if (req.body.wallet) updates.wallet = req.body.wallet;
    if (req.body.overall_ratings)
      updates.overall_ratings = req.body.overall_ratings;
    ...
    for (let field of imageFields) {
      if (req.files && req.files[field] && req.files[field][0]) {
        ...
        uploadPromises.push(
          uploadImageToCloudinary(req.files[field][0])
            .then((uploadResult) => {
            ...
            .catch((uploadError) => {
              ...
              return res
                .status(500)
                .json({ error: `Failed to upload ${field}` });
            })
        );
      }
    }
    ...
    const driver = await Drive.findByIdAndUpdate(req.params.id, updates, {
      new: true,
    });
